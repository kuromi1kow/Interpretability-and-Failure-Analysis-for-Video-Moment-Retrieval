...
class BaseOptions(object):
    ...
    def parse(self):
        ...
        results_root = os.environ.get('LIGHTHOUSE_RESULTS_ROOT', self.opt.results_dir)

        env_int_overrides = {
            'LIGHTHOUSE_BSZ': 'bsz',
            'LIGHTHOUSE_EVAL_BSZ': 'eval_bsz',
            'LIGHTHOUSE_NUM_WORKERS': 'num_workers',
            'LIGHTHOUSE_N_EPOCH': 'n_epoch',
        }
        for env_name, opt_name in env_int_overrides.items():
            if env_name in os.environ:
                self.opt[opt_name] = int(os.environ[env_name])

        if self.resume:
            self.opt.results_dir = os.path.join(results_root, self.model, f"{self.dataset}_finetune", self.feature)
        else:
            self.opt.results_dir = os.path.join(results_root, self.model, self.dataset, self.feature)
            if self.domain:
        ...
        shared_qv_dir = os.path.join(project_root, 'data', 'qvhighlights')

        if self.dataset == 'qvhighlight' and self.feature in ['clip', 'clip_slowfast', 'clip_slowfast_pann']:
            self.opt.train_path = os.path.join(shared_qv_dir, 'annotations', 'highlight_train_release.jsonl')
            self.opt.eval_path = os.path.join(shared_qv_dir, 'annotations', 'highlight_val_release.jsonl')

            if self.feature == 'clip_slowfast_pann':
                v_feat_dirs = [os.path.join(shared_qv_dir, 'hdf5', 'clip_slowfast_features.h5')]
                t_feat_dir = os.path.join(shared_qv_dir, 'txt_features', 'clip_text')
                a_feat_dirs = [os.path.join(shared_qv_dir, 'features', 'pann')]
                ...
                v_feat_dirs = [os.path.join(shared_qv_dir, 'hdf5', 'clip_features.h5')]
                t_feat_dir = os.path.join(shared_qv_dir, 'txt_features', 'clip_text')

        elif self.dataset == 'qvhighlight_pretrain':
